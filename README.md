...
wasm-bindgen --target web --out-dir doc/pkg --no-typescript target/wasm32-unknown-unknown/release/anagram_design.wasm
python3 -m http.server 8000 --directory doc             # then open http://localhost:8000
```
...
To publish on GitHub Pages, point Pages at the `doc/` directory so the bundled `pkg/` assets are served alongside `index.html`.
